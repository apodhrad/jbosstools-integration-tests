...
import org.jboss.tools.switchyard.reddeer.condition.TableIsNotEmpty;
...
public class BottomUpEJBTest extends SWTBotTestCase {
	...
	public void bottomUpCamelTest() throws Exception {
		...
		new WaitUntil(new TableIsNotEmpty());
		new PushButton("OK").click();
		Bot.get().shell("Bean Implementation").activate();
		...
		new TextEditor(JAVA_FILE + ".java").typeBefore("@Stateless", "@Service(Hello.class)")
				.deleteLineWith("HelloBean").type("public class HelloBean implements Hello {")
				.saveAndClose();
